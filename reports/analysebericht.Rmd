...
title: "Der Einfluss des Listenplatzes auf den Wahlerfolg: Analysebericht"
thanks: "Der Code und die Daten der Analysen sind auf Github abgelegt ([[Link]{.ul}](https://github.com/marcstoeckli/grossratswahlen_bs)). Ich freue mich über Kommentare und Anregungen via kontakt@marcstoeckli.ch."
author: "Marc Stöckli"
abstract: "In diesem Bericht untersuche ich den Einfluss des Listenrangs auf den Wahlerfolg bei den Grossratswahlen 2008 bis 2024 in Basel-Stadt. Meine Analysen zeigen, dass ein höherer Listenrang zu einem geringeren Wahlerfolg führt. Der Effekt tritt besonders bei der GLP, der SP, der SVP und der BastA! auf und variiert je nach Wahljahr und -kreis. Ich stelle einen deutlichen Anstieg des Listenrangeffekts in den Wahljahren 2020 und 2024 fest."
...
model_sp_kreis_share <- lm("share_scale ~ kreis*incumbent + year + kreis*gender + either + kreis*list_place", 
   data = data_final)

# Backlog

data_final %>% 
  mutate(rank_pred = predict(lm("rank ~ kreis*incumbent + year + gender + kreis*list_place", data = .))) %>% 
  ggplot() +
  geom_point(aes(x = list_place, y = -rank, col = gender, shape = incumbent)) +
  geom_line(aes(x = list_place, y = -rank_pred, col = gender, shape = incumbent)) +
...
Wenn die Reihenfolge nicht die Präferenz der Partei widerspiegelt, ist der Listenrangeffekt in der Regel unerwünscht. Das Wahl- und Abstimmungsreglement der SP Basel-Stadt legt beispielsweise fest, dass bei Listenwahlen zuerst die Frauen und anschliessend die Männer auf der Liste aufzuführen sind, jeweils in alphabetischer Reihenfolge. Dies führt dazu, dass Kandidierende mit Nachnamen hinten im Alphabet bei einem allfälligen Listenrangeffekt systematisch benachteiligt werden würden. Dieselbe Person würde zudem bei *jeder* Wahl benachteiligt werden, da sie aufgrund ihres Nachnamens immer einen schlechten Listenplatz hätte.

Für die Analysen in diesem Bericht untersuche ich die Daten der Grossratswahlen von Basel-Stadt der Jahre 2008, 2012, 2016, 2020 und 2024. Ich konzentriere mich dabei auf den Effekt des Listenrangs auf den Rang, den eine kandidierende Person in ihrem Wahlkreis auf ihrer Liste bei der Wahl erreicht hat (kurz \enquote{Wahlergebnisrang}). Wenn die gefundene Effektstärke 0 beträgt, hat der Listenrang keinen Einfluss auf den Wahlerfolg. Liegt die Effektstärke über 0, führt ein höherer Listenrang -- bei sonst gleichen Bedingungen -- im Durchschnitt zu einem schlechteren Wahlergebnisrang. Eine Effektstärke von 1 bedeutet, dass ein Platz weiter unten auf der Liste genau einen Rang schlechter im Wahlergebnis bedeutet. In diesem Bericht bezeichne ich eine Effektstärke ab 0.25 als hoch und ab 0.5 als sehr hoch.

Ich präsentiere die Ergebnisse für alle Listen/Parteien, beschränke mich für vertiefte Analysen aber auf die SP. Als Erweiterung untersuche ich zusätzlich den Effekt des Listenrangs auf den Anteil der Stimmen einer kandidierenden Person an den gesamten Stimmen für ihrer Liste in ihrem Wahlkreis (kurz \enquote{Stimmenanteil}).

Methodisch verwende ich die multivariate Regressionsanalyse. Dies ermöglicht es mir, den Effekt des Listenrangs von anderen Effekten -- dem Geschlecht, dem Alter, dem Bisherigen-Status, etc. -- zu unterscheiden. Ziel ist es, den *kausalen* Effekt des Listenrangs auf den Wahlerfolg zu finden.

Zusammenfassend finde ich in meinen Analysen Folgendes:

-   Bei fast allen Parteien **finde ich einen Listenrangeffekt**. Der Effekt ist am stärksten ausgeprägt bei der GLP, der SP, der SVP und der BastA! (in dieser Reihenfolge). Einzig bei der LDP gibt es keine Hinweise darauf, dass der Listenrang relevant für den Wahlerfolg ist.

-   Über alle Wahljahre und -kreise hinweg verliert bei der SP ein:e Kandidat:in pro Listenplatz weiter unten -- bei sonst gleichen Bedingungen -- im Durchschnitt **`r margins_sp["list_place"]`** **Wahlergebnisränge**.
...
-   Innerhalb der SP gibt es **deutliche Unterschiede des Effekts über die Wahlkreise**. Am stärksten ist der Listenrangeffekt in Riehen (**Effektstärke =** **`r margins_sp_kreis["Riehen"]`**) und Grossbasel Ost (**Effektstärke =** **`r margins_sp_kreis["Grossbasel Ost"]`**), gefolgt vom Kleinbasel (**Effektstärke =** **`r margins_sp_kreis["Kleinbasel"]`**). Der Listenrangeffekt in Riehen ist zwar gross, aber nicht statistisch signifikant, vermutlich aufgrund der kleinen Stichprobengrösse (wenig Sitze im Grossen Rat). In Grossbasel West kann ich keinen Effekt finden.

-   In Bezug auf die Geschlechter stelle ich fest, dass Frauen zwar die besseren Wahlergebnisse erzielen, diese aber statistisch gesehen durch ihre besseren Listenplätze erklärt werden können: Wird auf den Listenrang kontrolliert, **kann kein statistisch signifikanter Geschlechtereffekt mehr festgestellt werden**.

-   Werden nur die veränderten Wahlzettel betracht, so zeigt sich, dass der Listenrangeffekt **stärker ausgeprägt ist bei den Stimmen von veränderten Wahlzetteln der SP-Liste** im Vergleich zu den Stimmen von veränderten Wahlzetteln der anderen Listen (Panaschierstimmen). Anders ausgedrückt: Das Kumulieren und Streichen auf der SP-Liste führen zu einem stärkeren Listenrangeffekt als das Panaschieren von SP-Kandidierenden.

-   **Amtierende Grossrät:innen** schneiden deutlich besser ab als neue Kandidierende. Keinen statistisch signifikanten Effekt konnte ich hingegen für das **Alter** finden. Ebenso konnte ich keinen statistisch signifikanten Effekt finden, ob eine kandidierende Person auf der Liste **unter oder über einer amtierenden Kandidierenden** aufgeführt ist.

# Daten

Ich analysiere die Daten der Grossratswahlen in Basel-Stadt der Jahre 2008, 2012, 2016, 2020 und 2024. Für die Jahre 2020 und 2024 kann ich hierfür auf das Datenportal \enquote{Open Government Data} (OGD) zurückgreifen, über welches die Daten bereits aufbereitet und in hoher Qualität verfügbar sind. Für die Jahre 2008, 2012 und 2016 habe ich die Daten aus den PDFs der Wahlresultate und -protokolle erfasst. Da in diesen PDFs die Amtsinhaberschaft nicht erkennbar ist, habe ich zusätzlich die Datenbank der Ratsmitgliedschaften aus dem OGD verwendet, um die amtierenden Grossrät:innen zum Zeitpunkt der jeweiligen Wahl zu identifizieren.

Für die SP nutze ich die Daten aller Grossratswahlen von 2008 bis 2024. Für die restlichen Listen bzw. Parteien nutze ich lediglich die Daten der Wahlen von 2020 und 2024. Ich berücksichtige folgende Listen: BastA!, FDP, GLP, GRÜNE, Grünes Bündnis, LDP, Die Mitte/CVP und SVP. BastA! und GRÜNE sind in den Daten nur im Wahljahr 2024 vertreten, da 2020 die beiden Parteien noch als gemeinsame Liste \enquote{Gründes Bündnis} angetreten sind. Analog ist das Grüne Bündnis in den Daten nur 2020 vertreten. Die CVP hat zwischen den Wahljahren 2020 und 2024 ihren Namen zu \enquote{Die Mitte} geändert. Ich spreche einfachheitshalber in beiden Jahren von der Mitte. Alle weiteren Listen aus den Wahlen von 2020 und 2024 schliesse ich von der Analyse aus, da sie nicht vergleichbar sind.

Den Listenrang eine:r Kandidat:in bestimme ich anhand der letzten beiden Ziffern der Kandidierendennummer. Die Person mit der Nummer \enquote{0501} hat beispielsweise Listenrang 1, die Person mit der Nummer \enquote{0510} Listenrang 10. Da jede Kombination aus Wahljahr, Partei und Wahlkreis eigene Listenränge hat, kommt derselbe Listenrang mehrfach in den Daten vor (\enquote{Listenrang 1 bei den Grossratswahlen von 2024 der SP in *Grossbasel Ost*}, \enquote{Listenrang 1 bei den Grossratswahlen von 2024 der SP im *Grossbasel West*}, etc.). Ein höherer Listenrang beutet grundsätzlich, dass eine Person weiter unten auf dem Wahlzettel erscheint. Allerdings wird die obere Hälfte der Kandidierenden auf der linken Seite des Wahlzettels aufgeführt, die untere Hälfte auf der rechten Seite. Personen mit einem Listenplatz in der Mitte können daher sowohl oben (rechts) als auch unten (links) auf dem Wahlzettel stehen.

Ich untersuche den Einfluss des Listenrangs auf zwei Zielgrössen: Erstens den Rang, den eine kandidierende Person auf ihrer Liste im Wahlkreis erreicht (\enquote{Wahlergebnisrang}). Zweitens den Anteil der Stimmen, die eine kandidierende Person im Vergleich zu den Gesamtstimmen ihrer Liste im Wahlkreis erhält (\enquote{Stimmenanteil}). Den Stimmenanteil berechne ich ohne Zusatzstimmen, sodass die Summe der Stimmenanteile aller Kandidat:innen einer Liste in einem Wahlkreis genau 100% ergibt.[^1]

[^1]: Zusatzstimmen erhält eine Liste bei ungültigen Stimmen, gestrichenen Namen und leeren Linien.

Der Wahlergebnisrang und der Stimmenanteil eine:r Kandidat:in sind zwar hoch miteinander korreliert, dennoch kann der Einfluss des Listenrangs auf diese beiden Zielgrössen unterschiedlich sein. Welche Zielgrösse \enquote{korrekt} ist, hängt von der Perspektive ab. Der Effekt des Listenrangs auf den Stimmenanteil kann gering sein; wenn die Stimmenzahlen der Kandidierenden jedoch nahe beieinander liegen, kann dies für die Wahlwahrscheinlichkeit, gewählt zu werden, dennoch relevant sein. Da ich mich in dieser Analyse vorrangig für den Einfluss des Listenrangs auf die Wahlwahrscheinlichkeit interessiere, konzentriere ich mich auf den Wahlergebnisrang. Die Ergebnisse für den Stimmenanteil zeige ich im Anhang.

## Deskriptive Analyse

Die unteren Abbildungen zeigen den Zusammenhang zwischen dem Listenrang (X-Achse) und dem Wahlergebnisrang (Y-Achse) für die SP, jeweils separat für die Wahlkreise Kleinbasel, Grossbasel Ost, Grossbasel West und Riehen. Jeder Punkt ist eine kandidierende Person in dem jeweiligen Wahlkreis und Jahr. Je weiter rechts eine Person auf der X-Achse liegt, desto höher ist ihr Listenrang; je weiter oben ein Punkt auf der Y-Achse liegt, desto niedriger ist der Wahlergebnisrang, d.h. desto besser ist das Wahlergebnis dieser Person. Ein (nicht zwingend kausaler) Listenrangeffekt zeigt sich in den Abbildungen dadurch, dass das Wahlergebnis in Durchschnitt schlechter ausfällt, je weiter rechts eine Person liegt. Die blaue Regressionslinie visualisiert diesen Zusammenhang. Die Regressionslinie wird in jedem Jahr und Wahlkreis so durch die Punktewolke gezogen, dass sie die Summe der (quadrierten) vertikalen Abstände zu den Punkten minimiert.

Frauen (blau) und Männer (grün) sind farblich hervorgehoben. Dabei zeigt sich die eingangs erwähnte Regelung der SP Basel-Stadt, dass zuerst die Frauen und anschliessend die Männer auf der Liste aufzuführen sind. Ebenfalls hervorgehoben sind die amtierenden Grossrät:innen (Dreiecke) und die neuen Kandidierenden (Kreise). Die amtierenden Grossrät:innen weisen über alle Wahlkreise und Jahre die deutlich besseren Wahlergebnisse aus. Dieser Effekt ist wohlbekannt, da er bei jeder Grossratswahl so offensichtlich und stark ausgeprägt ist.

Beim Betrachten der Abbildung und insbesondere der Regressionslinien lässt sich in mehreren Wahlkreisen ein Zusammenhang zwischen dem Listenrang und dem Wahlergebnisrang erkennen. Ebenso zeigt sich, dass dieser Zusammenhang über die Jahre zugenommen hat und deutliche Unterschiede zwischen den Wahlkreisen bestehen. Besonders stark ist der Effekt in den Jahren 2020 und 2024. 2008 war der Zusammenhang in den meisten Wahlkreisen hingegen noch weniger stark ausgeprägt (in Grossbasel West ist der Effekt sogar gegenteilig).

Aus diesem Zusammenhang kann jedoch noch kein kausaler Listenrangeffekt ableitet werden, da es noch andere Effekte geben kann, die das Ergebnis verzerren. In Grossbasel Ost sind 2024 beispielsweise überproportional viele amtierende Grossrät:innen mit niedrigem Listenrang angetreten. Diese Grossrät:innen haben erwartungsgemäss das beste Wahlergebnis erzielt, jedoch vermutlich wegen ihrer Amtsinhaberschaft und nicht wegen ihrem Listenrang. Die Regressionslinie (die den Listenrangeffekt widerspiegelt) ist dadurch zu \enquote{steil}, weil sie bei den niedrigen Listenrängen von dem guten Wahlergebnis der amtierenden Grossrät:innen nach oben \enquote{gezerrt} wird. Das Gegenteil ist in Grossbasel West 2016 zu beobachten. Die vier amtierenden Grossrät:innen mit den hohen Listenrängen zerren die Regressionslinie so stark nach oben, dass die Regressionslinie sogar eine gegenteilige Steigung aufweist. Mit einem statistischen Modell können (und müssen) diese anderen Effekte \enquote{herausgerechnet} werden. Die von mir verwendeten Modelle werden in den kommenden Kapiteln beschrieben.

Im Anhang zeige ich die Abbildung der Jahre 2020 und 2024 für die restlichen Parteien bzw. Listen. Bei den anderen Parteien zeigt sich ein noch deutlicherer Listenrangeffekt, insbesondere bei der BastA!, den Grünen (inkl. dem Grünen Bündnis) und der GLP.

```{r fig_1, fig.cap="Listen- und Wahlergebnisrang -- Kleinbasel und Grossbasel Ost", echo=FALSE, message=FALSE, warning=FALSE, fig.width=6, fig.height=8, fig.align='center', fig.pos='H'}
plots <- list()
for(i in c("Kleinbasel", "Grossbasel Ost")) {
  plot <- data_final %>% 
    mutate(incumbent = factor(incumbent, levels = c(TRUE, FALSE))) %>% 
    filter(kreis == i) %>% 
    mutate(rank_pred = predict(lm(rank ~ year*list_place, 
                                data = .))) %>%
    ggplot(aes(x = list_place, y = rank)) +
    geom_point(aes(col = gender, shape = incumbent)) +
    geom_line(aes(x = list_place, y = rank_pred), col = "blue") +
...
for (i in seq(1, length(plots), by = 2)) {
  grid.arrange(grobs = plots[i:(i+1)], ncol = 2)
}
```

```{r fig_2, fig.cap="Listen- und Wahlergebnisrang -- Grossbasel West und Riehen", echo=FALSE, message=FALSE, warning=FALSE, fig.width=6, fig.height=8, fig.align='center', fig.pos='H'}
plots <- list()
...
-   $Rang_{it}$: Listenrang von Kandidat:in $i$ im Wahljahr $j$
...
margins(model) %>%
  tidy() %>%
  mutate(id = factor(term, levels = c("genderM", "incumbent", "kreisGrossbasel West", "kreisKleinbasel", "kreisRiehen", "year2012", "year2016", "year2020", "year2024", "list_place"))) %>% 
  arrange(id) %>%
...
margins(model_sp_ownothers, variables = "list_place", 
  ...
  select(`Wahlkreis`, `Marginaler Effekt`, `Standardfehler`, 
         `p-Wert`, `Signifikanz`) %>% 
  kbl("latex", digits = 3, 
      caption = "Durchschnittliche marginale Effekte des Listenrangs nach Wahlkreisen (Wahljahre 2020 und 2024)", booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"),
                full_width = FALSE) %>%
  column_spec(1, width = "3cm") %>%
  column_spec(2, width = "4.25cm") %>%
...
margins(model_sp_own, variables = "list_place", 
  ...
  select(`Wahlkreis`, `Marginaler Effekt`, `Standardfehler`, 
         `p-Wert`, `Signifikanz`) %>% 
  kbl("latex", digits = 3, 
      caption = "Durchschnittliche marginale Effekte des Listenrangs der SP-Stimmen nach Wahlkreisen (Wahljahre 2020 und 2024)", booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"),
                full_width = FALSE) %>%
  column_spec(1, width = "3cm") %>%
  column_spec(2, width = "4.25cm") %>%
...
margins(model_sp_others, variables = "list_place", 
  ...
  select(`Wahlkreis`, `Marginaler Effekt`, `Standardfehler`, 
         `p-Wert`, `Signifikanz`) %>% 
  kbl("latex", digits = 3, 
      caption = "Durchschnittliche marginale Effekte des Listenrangs der Panaschierstimmen nach Wahlkreisen (Wahljahre 2020 und 2024)", booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"),
                full_width = FALSE) %>%
  column_spec(1, width = "3cm") %>%
  column_spec(2, width = "4.25cm") %>%
...
Ich untersuche, ob auch weitere Variablen eine Effekt auf den Wahlerfolg haben.
...
margins(model) %>%
  ...
  select(`Variable`, `Marginaler Effekt`, `Standardfehler`, 
         `p-Wert`, `Signifikanz`) %>% 
  kbl("latex", digits = 3, 
      caption = "Durchschnittliche marginale Effekte weiterer erklärender Variablen", booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"),
                full_width = FALSE) %>%
  column_spec(1, width = "4.75cm") %>%
  column_spec(2, width = "4.25cm") %>%
...
Für die Wahljahre 2020 und 2024 untersuche ich zusätzlich den Listenrangeffekt für die BastA!, FDP, GLP, GRÜNE, Grünes Bündnis, LDP, Die Mitte, SP und SVP.

Ich verwende erneut das Basismodell und nehme zusätzlich eine Variable für die Partei der kandidierenden Person $Partei_{it}$ auf sowie Interaktionen aus der Partei mit dem Bisherigen-Status ($Partei \times Bisher_{it}$), dem Geschlecht ($Partei \times Geschlecht_{it}$) und dem Listenrang ($Partei \times Rang_{it}$).
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=6, fig.height=8, fig.align='center', fig.pos='H'}
plots <- list()
for(i in c("Kleinbasel", "Grossbasel Ost", "Grossbasel West", "Riehen")) {
  plot <- data_final %>% 
    mutate(incumbent = factor(incumbent, levels = c(TRUE, FALSE))) %>% 
    filter(kreis == i) %>% 
    mutate(share_pred = predict(lm(share ~ year*list_place, 
                                data = .))) %>%
    ggplot(aes(x = list_place, y = share)) +
    geom_point(aes(col = gender, shape = incumbent)) +
    geom_line(aes(x = list_place, y = share_pred), col = "blue") +
    scale_color_manual(values = c("M" = "#72cb8c", "F" = "#66a3c2"),
                       labels = c("M" = "M", "F" = "F")) +
    scale_shape_manual(values = c("TRUE" = 17, "FALSE" = 16),
                       labels = c("TRUE" = "Ja", "FALSE" = "Nein")) +
    ...
    labs(title = i, x = "Stimmenanteil", y = "Wahlergebnisrang") +
    theme_minimal() +
    guides(color = guide_legend(title = "Geschlecht"),
           shape = guide_legend(title = "Amtierend")) +
  theme(legend.position = "bottom",
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=6, fig.height=8, fig.align='center', fig.pos='H'}
plot_gb <- data_final_all %>% 
  mutate(incumbent = factor(incumbent, levels = c(TRUE, FALSE))) %>% 
  filter(party == "GB") %>% 
   mutate(rank_pred = predict(lm(rank ~ kreis*list_place, 
                                data = .))) %>%
  ggplot(aes(x = list_place, y = rank)) +
  geom_point(aes(col = gender, shape = incumbent)) +
  geom_line(aes(x = list_place, y = rank_pred), col = "blue") +
  ...
  scale_x_continuous(breaks = seq(1, 34, 4), limits = c(1, 35)) +
  scale_y_reverse(breaks = seq(1, 34, 8), limits = c(35, 1)) +
  facet_wrap(~ kreis + year, ncol = 1) +
  labs(title = "Grünes Bündnis", x = "Listenrang", y = "Wahlergebnisrang") +
  theme_minimal() +
  guides(color = guide_legend(title = "Geschlecht"),
         shape = guide_legend(title = "Amtierend")) +
  theme(legend.position = "bottom",
...
plot_gruene <- data_final_all %>% 
  ...
  scale_y_reverse(breaks = seq(1, 34, 8), limits = c(35, 1)) +
  facet_wrap(~ kreis + year, ncol = 1) +
  labs(title = "Grüne", x = "Listenrang", y = "Wahlergebnisrang") +
  theme_minimal() +
  guides(color = guide_legend(title = "Geschlecht"),
         shape = guide_legend(title = "Amtierend")) +
  theme(legend.position = "bottom",
...
plot_basta <- data_final_all %>% 
  ...
  scale_y_reverse(breaks = seq(1, 34, 8), limits = c(35, 1)) +
  facet_wrap(~ kreis + year, ncol = 1) +
  labs(title = "BastA", x = "Listenrang", y = "Wahlergebnisrang") +
  theme_minimal() +
  guides(color = guide_legend(title = "Geschlecht"),
         shape = guide_legend(title = "Amtierend")) +
  theme(legend.position = "bottom",
...
grid.arrange(plot_gb, plot_gruene, plot_basta, ncol = 3)
```
...
data_final_all %>% 
  mutate(incumbent = factor(incumbent, levels = c(TRUE, FALSE))) %>% 
  filter(party == "GLP") %>% 
  mutate(rank_pred = predict(lm(rank ~ year*kreis*list_place, 
                                data = .))) %>%
  ggplot(aes(x = list_place, y = rank)) +
  geom_point(aes(col = gender, shape = incumbent)) +
  geom_line(aes(x = list_place, y = rank_pred), col = "blue") +
  ...
  scale_x_continuous(breaks = seq(1, 34, 4), limits = c(1, 35)) +
  scale_y_reverse(breaks = seq(1, 34, 8), limits = c(35, 1)) +
  facet_wrap(~ kreis + year, ncol = 2) +
  labs(title = "GLP", x = "Listenrang", y = "Wahlergebnisrang") +
  theme_minimal() +
  guides(color = guide_legend(title = "Geschlecht"),
         shape = guide_legend(title = "Amtierend")) +
  theme(legend.position = "bottom",
...
data_final_all %>% 
  ...
  scale_y_reverse(breaks = seq(1, 34, 8), limits = c(35, 1)) +
  facet_wrap(~ kreis + year, ncol = 2) +
  labs(title = "FDP", x = "Listenrang", y = "Wahlergebnisrang") +
  theme_minimal() +
  guides(color = guide_legend(title = "Geschlecht"),
         shape = guide_legend(title = "Amtierend")) +
  theme(legend.position = "bottom",
...
data_final_all %>% 
  ...
  scale_y_reverse(breaks = seq(1, 34, 8), limits = c(35, 1)) +
  facet_wrap(~ kreis + year, ncol = 2) +
  labs(title = "LDP", x = "Listenrang", y = "Wahlergebnisrang") +
  theme_minimal() +
  guides(color = guide_legend(title = "Geschlecht"),
         shape = guide_legend(title = "Amtierend")) +
  theme(legend.position = "bottom",
...
data_final_all %>% 
  ...
  scale_y_reverse(breaks = seq(1, 34, 8), limits = c(35, 1)) +
  facet_wrap(~ kreis + year, ncol = 2) +
  labs(title = "Die Mitte", x = "Listenrang", y = "Wahlergebnisrang") +
  theme_minimal() +
  guides(color = guide_legend(title = "Geschlecht"),
         shape = guide_legend(title = "Amtierend")) +
  theme(legend.position = "bottom",
...
data_final_all %>% 
  ...
  scale_y_reverse(breaks = seq(1, 34, 8), limits = c(35, 1)) +
  facet_wrap(~ kreis + year, ncol = 2) +
  labs(title = "SVP", x = "Listenrang", y = "Wahlergebnisrang") +
  theme_minimal() +
  guides(color = guide_legend(title = "Geschlecht"),
         shape = guide_legend(title = "Amtierend")) +
  theme(legend.position = "bottom",
